...
(defn add-receiver*
  ...
  (swap! receivers
         (fn [rv]
           (when-let [cb (get-in rv [port k])]
             (-remove-receiver port cb))
           (-add-receiver port callback)
           (update rv port assoc k callback)))
  k)
...
(defn add-receiver
  ...
  "
  [port k callback]
  (add-receiver* port k (fn [msg millis] (callback (event msg) millis))))
...
(defn remove-receiver
  ...
  (swap! receivers
         (fn [rv]
           (if-let [cb (get-in rv [port k])]
             (do
               (-remove-receiver port cb)
               (update rv port dissoc k))
             rv)))
  k)
